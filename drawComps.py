...
class Component:
    ...
    def save(self):
        cv2.imwrite('component_%d.jpg' % self.id, self.img)
...
class Resistor(Component):
    ...
    """
    def __init__(self, length, theta, error = 20, id=1):
...
if __name__ == "__main__":
    ...
    cap = Capacitor(error = 20, id=0)
    cap.draw()
    cap.show()
    cap.save()
    
    #Theta math.pi/5 is the angle on the resistors
    res = Resistor(100, math.pi/5, error = 30, id=1)
    res.draw()
    res.show()
    res.save()
